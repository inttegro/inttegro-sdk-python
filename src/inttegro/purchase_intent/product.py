"""Product in the ``inttegro.purchase_intent`` resource namespace.
...
class Product(ApiModel):
    """Typed product data in the purchase intent resource namespace.
    ...
    API contract schema: ``PurchaseIntentProduct``.
    """
    ...
    """Product attributes. Optional; nullable. Python type: ``list[PurchaseIntentProductAttributesItem] | None``; wire name: ``attributes``; JSON type: array of object values"""
    ...
    """Non-archived catalog prices currently attached to the product. Optional; nullable. Python type: ``list[ProductPriceSummary] | None``; wire name: ``prices``; JSON type: array of object (ProductPriceSummary) values"""
    variant_set_id: str | None = field(init=False)
    """Variant set configured for this Buy link. Optional; nullable. Python type: ``str | None``; wire name: ``variant_set_id``; JSON type: string"""
...
from inttegro.purchase_intent.product_attributes_item import ProductAttributesItem as PurchaseIntentProductAttributesItem
